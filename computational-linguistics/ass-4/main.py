...
# @Last Modified time: 2021-01-14 03:48:18
...
def data_loader(e_file, f_file, args):
    ...
    # ]
    #************************************************
    sentence_pairs = [[sentence.strip().split() for sentence in pair] for pair in zip(open(f_file), open(e_file))][:args.num_sents]
    english_words = []
    foreign_words = []
...
def perplexity_plot(ppxty, args):
    # Plot of log base 2 of perplexity
...
def main():
    ...
    start = time.time()
    sentence_pairs, english_words, foreign_words = data_loader(args.eng_f, args.foreign_f, args)
    end =  time.time()
    ...
    if args.alpha:
        ...
        alpha = None
    ibm_model = IBM_model(english_words, foreign_words, sentence_pairs, alpha, save_model)
...
def parse_arguments():
    ...
    parser.add_argument("foreign_f", help="path to target (foriegn) file ")
    ...
    parser.add_argument("-alpha", default=0.30, type=float, help='threshold score of translation probability for alignment')
